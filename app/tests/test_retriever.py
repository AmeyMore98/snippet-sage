...
from app.rag.retriever import Hit, fuse, normalize_scores
...
class TestNormalizeScores:
    ...
    def test_normalize_identical_scores(self):
        ...
        hits = [Hit("chunk1", 0.7, "preview1"), Hit("chunk2", 0.7, "preview2"), Hit("chunk3", 0.7, "preview3")]
        result = normalize_scores(hits)
        assert len(result) == 3
    ...
    def test_normalize_different_scores(self):
        ...
        hits = [
            Hit("chunk1", 0.1, "preview1"),  # min -> 0.0
            Hit("chunk2", 0.5, "preview2"),  # mid -> 0.5
            Hit("chunk3", 0.9, "preview3"),  # max -> 1.0
        ]
        result = normalize_scores(hits)

        assert len(result) == 3
        assert result[0].score == 0.0  # (0.1 - 0.1) / (0.9 - 0.1)
        assert result[1].score == 0.5  # (0.5 - 0.1) / (0.9 - 0.1)
        assert result[2].score == 1.0  # (0.9 - 0.1) / (0.9 - 0.1)
    ...
    def test_normalize_preserves_monotonic_order(self):
        ...
        hits = [Hit("chunk1", 10.5, "preview1"), Hit("chunk2", 15.2, "preview2"), Hit("chunk3", 12.8, "preview3")]
        result = normalize_scores(hits)

        # Original order: chunk1 (lowest), chunk3 (middle), chunk2 (highest)
        # Normalized scores should maintain this relative ordering
        assert result[0].score < result[2].score < result[1].score
        assert result[1].score == 1.0  # max score
        assert result[0].score == 0.0  # min score
...
class TestFuse:
    ...
    def test_fuse_one_empty_list(self):
        ...
        vector_hits = [Hit("chunk1", 0.8, "preview1"), Hit("chunk2", 0.6, "preview2")]
        lexical_hits = []
        ...
        assert result[0].chunk_id == "chunk1"
        assert result[0].score == 0.6 * 0.8  # 0.48
        assert result[1].chunk_id == "chunk2"
        assert result[1].score == 0.6 * 0.6  # 0.36

    def test_fuse_no_overlap(self):
        """Fusing non-overlapping results should combine all hits."""
        vector_hits = [Hit("chunk1", 0.8, "preview1")]
        lexical_hits = [Hit("chunk2", 0.9, "preview2")]

        result = fuse(vector_hits, lexical_hits, alpha=0.6, k=10)

        assert len(result) == 2
        # Results should be sorted by combined score (descending)
        chunk_scores = {hit.chunk_id: hit.score for hit in result}
        assert chunk_scores["chunk1"] == 0.6 * 0.8  # 0.48
        assert chunk_scores["chunk2"] == 0.4 * 0.9  # 0.36

    def test_fuse_with_overlap(self):
        """Fusing overlapping results should combine scores properly."""
        vector_hits = [Hit("chunk1", 1.0, "preview1"), Hit("chunk2", 0.5, "preview2")]
        lexical_hits = [Hit("chunk1", 0.8, "preview1"), Hit("chunk3", 0.6, "preview3")]
    ...
    def test_fuse_respects_k_limit(self):
        ...
        vector_hits = [Hit("chunk1", 1.0, "preview1"), Hit("chunk2", 0.8, "preview2"), Hit("chunk3", 0.6, "preview3")]
        lexical_hits = [Hit("chunk4", 0.9, "preview4"), Hit("chunk5", 0.7, "preview5")]
    ...
    def test_fuse_sorts_by_combined_score(self):
        """Results should be sorted by combined score descending."""
        vector_hits = [Hit("chunk1", 0.5, "preview1")]
        lexical_hits = [
            Hit("chunk2", 1.0, "preview2"),  # This should rank higher
            Hit("chunk3", 0.3, "preview3"),
        ]
    ...
    def test_fuse_alpha_weighting(self):
        ...
        vector_hits = [Hit("chunk1", 1.0, "preview1")]  # Strong vector match
        lexical_hits = [Hit("chunk2", 1.0, "preview2")]  # Strong lexical match
...
class TestHybridSearchLogic:
    ...
    def test_hit_named_tuple_creation(self):
        ...
        hit = Hit("chunk_id_123", 0.85, "Some preview text...")

        assert hit.chunk_id == "chunk_id_123"
        assert hit.score == 0.85
        assert hit.text_preview == "Some preview text..."
    ...
    def test_hit_with_optional_preview(self):
        ...
        assert hit.chunk_id == "chunk_id_123"
        assert hit.score == 0.85
        assert hit.text_preview is None
